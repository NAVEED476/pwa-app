...
const Register = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("offline", handleOnlineStatus); // Add event listener for offline status
    handleOnlineStatus();

    return () => {
      window.removeEventListener("online", handleOnlineStatus);
      window.removeEventListener("offline", handleOnlineStatus); // Remove event listener when component unmounts
    };
  }, []);
  ...
  const isFormValid = () => {
    return Object.values(formData).every((value) => value !== "") && emailValid;
  };
